...
def searсh_men(personInfo: list):
    ...
    cursor = conn.cursor()
    cursor.execute(f"SELECT S.ID, S.name, S.firstname, S.lastname, S.dateOfBirth, S.gender, N.name_pos, N.division"
                   f" FROM worker as S INNER JOIN position as N ON S.position = N.ID WHERE ({str_pers_info});")
